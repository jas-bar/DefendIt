...
public class Frustum implements ICulling {
    ...
    private boolean isBoxInFrustrum(float x, float y, float z,float w, float h, float d) {
        for (int i = 0; i < 6; i++) {
            /*
             * if( frustum[i][0] * (x) + frustum[i][1] * (y) + frustum[i][2] *
             * (z) + frustum[i][3] > 0 )
             * continue;
             */

            if (frustum[i][0] * (x - w) + frustum[i][1] * (y - h) + frustum[i][2]
                    * (z - d) + frustum[i][3] > 0)
                continue;
            if (frustum[i][0] * (x + w) + frustum[i][1] * (y - h) + frustum[i][2]
                    * (z - d) + frustum[i][3] > 0)
    ...
    public boolean blockRenders(int blockX, int blockY, int blockZ) {
        ...
        return isBoxInFrustrum(x, y, z, BlockRenderer.BLOCK_SIZE, BlockRenderer.BLOCK_SIZE, BlockRenderer.BLOCK_SIZE);
        /*
        for (int i = 0; i < 6; i++) {
            ...
            if (frustum[i][0] * (x - BlockRenderer.BLOCK_SIZE) + frustum[i][1] * (y - BlockRenderer.BLOCK_SIZE) + frustum[i][2]
                    * (z - BlockRenderer.BLOCK_SIZE) + frustum[i][3] > 0)
                continue;
            if (frustum[i][0] * (x + BlockRenderer.BLOCK_SIZE) + frustum[i][1] * (y - BlockRenderer.BLOCK_SIZE) + frustum[i][2]
                    * (z - BlockRenderer.BLOCK_SIZE) + frustum[i][3] > 0)
                continue;
    ...
    private void normalize(int plane) {
        float t = (float) Math.sqrt(frustum[plane][0] * frustum[plane][0] + frustum[plane][1] * frustum[plane][1] + frustum[plane][2]
                * frustum[plane][2]);
        frustum[plane][0] /= t;
        frustum[plane][1] /= t;
        frustum[plane][2] /= t;
    ...
    public boolean chunkRenders(int cx, int cz) {
        float x = cx*Chunk.SIZE_X*BlockRenderer.BLOCK_SIZE;
        float y = 0;
        float z = cz*Chunk.SIZE_Z*BlockRenderer.BLOCK_SIZE;
        
        return isBoxInFrustrum(x, y, z, Chunk.SIZE_X*BlockRenderer.BLOCK_SIZE, World.SIZE_Y*BlockRenderer.BLOCK_SIZE, Chunk.SIZE_Z*BlockRenderer.BLOCK_SIZE);        
    }
